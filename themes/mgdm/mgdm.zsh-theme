...
PROMPT='%{${fg[blue]}%}${USER}%{$reset_color%}@%{${fg[blue]}%}%m%{$reset_color%}:%{${fg[blue]}%}%1d%{${fg[$CARETCOLOR]}%}%#%{${reset_color}%} '
...
ZSH_THEME_VIRTUAL_ENV_WARNING_PREFIX="%{$fg[blue]%}venv:%{$fg[red]%}"
ZSH_THEME_VIRTUAL_ENV_WARNING_SUFFIX="%{$fg[blue]%}%{$reset_color%}"

MODE_INDICATOR="%{$fg[magenta]%}<%{$reset_color%}%{$fg[magenta]%}<<%{$reset_color%}"
